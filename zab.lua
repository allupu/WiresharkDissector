...
dissectClient = function (buf, pkt, tree)
	length = buf(0, 4):int()
	if length >= MAX_REQUEST_SIZE or NEGATIVE_XIDS[length] then
		xid = length
		length = 0
	elseif length == 0 then
      dissectConnectRequest(buf, tree, 0)
	  return
    elseif length < 0 then
		return 0
    else
		xid = buf(0, 4):int()
        if NEGATIVE_XIDS[xid] and xid < 0 then
			return 0
		elseif xid == 0 or xid == 1 then
			dissectConnectRequest(buf, tree, 4)
			return
        end
	end
	...
	if (buf(4, 4):int() == 0) then --cannot insert into a table at index 0, so this transaction id should be handled separately
		requestsXids[client]["0"] = i_opCode
	elseif not (i_opCode == 11 or i_opCode == -11) then --any request that is not a ping or a close should be tracked
		table.insert(requestsXids[client], buf(4, 4):int(), i_opCode)
	end
	...
	if (offset < buf:len() - 4) then
		...
		if i_opCode == 3 or i_opCode == 4 or i_opCode == 8 or i_opCode == 12 then
			tree:add(f_watch, buf(offset, 4))
			offset = offset + 4
		end
		
		if i_opCode == 0 then --CONNECT request
			dissectConnectRequest(buf, tree, offset)
		elseif i_opCode == 1 then --CREATE request
...
dissectConnectRequest = function(buf, tree, offset)
	tree:add(f_protoversion, buf(offset, 4))
	tree:add(f_timeout, buf(offset + 4, 4))
	local zxid = tree:add(f_zxid, buf(offset + 8, 8))
	zxid:add(f_zxid_epoch, buf(offset + 8, 4))
	zxid:add(f_zxid_count, buf(offset + 12, 4))
	tree:add(f_session, buf(offset + 16, 8))
	offset = offset + 24
end
